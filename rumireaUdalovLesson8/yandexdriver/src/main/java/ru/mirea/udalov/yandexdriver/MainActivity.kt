...
class MainActivity : AppCompatActivity(), DrivingSession.DrivingRouteListener {
    ...
    private var ROUTE_START_LOCATION = Point(55.548273, 37.528684)
    private var ROUTE_END_LOCATION = Point(55.726882, 37.579654)
    ...
    private fun submitRequest() {
        ...
        if (task) {
            ROUTE_START_LOCATION = Point( 55.548273, 37.528684 )
            ROUTE_END_LOCATION = Point(55.726882, 37.579654)
        }
    ...
    override fun onDrivingRoutes(list: List<DrivingRoute>) {
        ...
        if (task) {
            ...
            marker.addTapListener { _, _ ->
                Toast.makeText(applicationContext, "Фрунзенская", Toast.LENGTH_SHORT).show()
                false
            }
        }
    }
